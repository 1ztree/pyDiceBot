...
class Initiative:
	...
	# addInitiative : String -> Void
	def addInitiative(self, expr):
		group_size = 0
		m = re.match("#Roll ([1-9][0-9]?)?\s?(.*)?\s?([1-9][0-9]?)D([1-9][0-9]?)\s?\+?\s?([1-9][0-9]?)?", expr)
	...
	#Updates the initiative list when a new round starts and sorts the list even if a new round has not begun.
	# updateInitiative : Boolean -> Void
